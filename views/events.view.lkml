view: events {
  ...
  # NEED TO ADJUST CAMPAIGN MAPPING TO USE COMBINATION OF WEB AND MOBILE ATTRIBUTION
  # I DON'T SEE CAMPAIGN/SOURCE DATA IN ANDROID OR WEB SOURCE THOUGH - NEED TO EMAIL RICKY FROM SEGMENT
  ...
  derived_table: {
    ...
    sql:
      ...
      --join the unioned track and pages with id_map to fill in user_id retroactively for identified users
        select e.id, e.received_at, e.cp_id, e.event,
          ...
          coalesce(e.user_id, i.user_id, e.cp_id) as visitor_id
        from events e
