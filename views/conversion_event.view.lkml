  ...
  derived_table: {
    ...
    sql:
      ...
      web as (
        ...
        from @{segment_web_schema_name}.@{conversion_event}
      ),
      ios as (
        select
          id,
          ...
          @{conversion_value} as conversion_value,
          row_number() over (partition by id order by loaded_at desc) as rn
        from @{segment_ios_schema_name}.@{conversion_event}
      ),
      android as (
        select
          id,
          ...
          @{conversion_value} as conversion_value,
          row_number() over (partition by id order by loaded_at desc) as rn
        from @{segment_android_schema_name}.@{conversion_event}
      ),
      ce as (
        select *, 'Web' as platform from web where rn=1
        union all
        select *, 'iOS' as platform from ios where rn=1
        union all
        select *, 'Android' as platform from android where rn=1
      )
      select *,
